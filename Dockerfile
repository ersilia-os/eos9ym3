...
RUN conda create -n eos2re5-py27 python=3.7 -y
RUN conda install -n eos2re5-py27 -c rdkit -y
...
RUN CONDA_PATH=$(dirname $(dirname $(which conda))) && PYTHON_ENV_PATH="${CONDA_PATH}/envs/eos2re5-py27/bin/python" && $PYTHON_ENV_PATH -m pip install scikit-learn==0.17.1 && $PYTHON_ENV_PATH -m pip install scipy==1.2.3 && $PYTHON_ENV_PATH -m pip install rdkit
